-- Admin Role Management System (Simplified Version)
-- This migration adds admin functionality without dependencies on reviews or audit_log tables
...
BEGIN
  ...
  SELECT 
    ...
    CASE 
      WHEN EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'transcriptions') 
      THEN COALESCE((SELECT COUNT(*) FROM transcriptions WHERE transcriptionist_id = up.id), 0)
      ELSE 0
    END AS transcription_count,
...
-- Add trigger for transcriptions table if it exists
DO $$
BEGIN
