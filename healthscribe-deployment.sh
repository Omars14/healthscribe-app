...
# HealthScribe.Pro - Complete VPS Deployment Script
...
# Configuration for healthscribe.pro
...
SUPABASE_DIR="/opt/supabase-healthscribe"
...
show_credentials() {
    echo
    echo "==============================================="
    echo "🔐 HEALTHSCRIBE.PRO - ACCESS CREDENTIALS"
    echo "==============================================="
    echo
    ...
    echo "   Host: localhost"
    echo "   Port: 5432"
    echo "   Database: medical_transcription"
...
update_system() {
    ...
    apt install -y curl wget git unzip software-properties-common ufw htop iotop sysstat certbot python3-certbot-nginx
...
setup_monitoring() {
    ...
    mkdir -p /var/log/healthscribe
    chown www-data:www-data /var/log/healthscribe

    # Set up log rotation
    cat > /etc/logrotate.d/healthscribe << EOF
/var/log/healthscribe/*.log {
    daily
    missingok
    rotate 52
    compress
...
EOF
    ...
    (crontab -l ; echo "0 4 * * * /usr/local/bin/backup-healthscribe.sh") | crontab -
...
test_deployment() {
    ...
    # Test 8: PM2 status
    if pm2 list | grep -q "healthscribe"; then
...
show_completion_summary() {
    echo
    echo "==============================================="
    echo "🎉 HEALTHSCRIBE.PRO - DEPLOYMENT COMPLETE!"
    echo "==============================================="
    echo
    log_success "Your complete medical transcription system is ready!"
    echo
    ...
    echo
    echo "   n8n Dashboard Username: admin"
    echo "   n8n Dashboard Password: HealthScribe2025!"
    echo
    ...
    echo "4. 📝 Import the medical transcription workflow"
    ...
    echo "   - Self-Hosted Guide: README_SELF_HOSTED.md"
    echo "   - Supabase Setup: SUPABASE_SELF_HOSTED_GUIDE.md"
    echo "   - n8n Setup: N8N_VPS_SETUP_GUIDE.md"
    echo
    echo "==============================================="
    echo "🚀 Your HealthScribe.Pro system is ready for production!"
    echo "==============================================="
}

main() {
    echo "==============================================="
    echo "🩺 HealthScribe.Pro - Complete VPS Deployment"
    echo "==============================================="
    echo
    log_info "This script will set up your complete medical transcription system:"
    ...
    log_warning "VPS IP: 154.26.155.207"
    log_warning "Make sure your DNS records point to this IP address!"
    echo

    check_root
    show_credentials

    read -p "Have you configured your DNS records to point to 154.26.155.207? (y/N): " -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        log_warning "Please configure your DNS records first:"
        echo "  healthscribe.pro     A     154.26.155.207"
        echo "  supabase.healthscribe.pro     A     154.26.155.207"
        echo "  n8n.healthscribe.pro     A     154.26.155.207"
        echo
