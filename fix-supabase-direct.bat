...
echo y | plink -ssh -pw Nomar123 root@154.26.155.207 "docker stop \$(docker ps -q --filter 'name=supabase') 2>/dev/null || true"
echo y | plink -ssh -pw Nomar123 root@154.26.155.207 "docker rm \$(docker ps -aq --filter 'name=supabase') 2>/dev/null || true"
echo y | plink -ssh -pw Nomar123 root@154.26.155.207 "docker system prune -f"

echo Step 2: Creating environment configuration...
echo y | plink -ssh -pw Nomar123 root@154.26.155.207 "cat > /opt/supabase/supabase/docker/.env << 'EOF'
...
echo y | plink -ssh -pw Nomar123 root@154.26.155.207 "cat > /opt/supabase/supabase/docker/docker-compose-fixed.yml << 'EOF'
version: '3.8'
services:
  db:
...
echo y | plink -ssh -pw Nomar123 root@154.26.155.207 "cat > /opt/supabase/supabase/docker/kong.yml << 'EOF'
_format_version: \"1.1\"
services:
  - name: auth-v1-open
    url: http://auth:9999/
...
echo y | plink -ssh -pw Nomar123 root@154.26.155.207 "cd /opt/supabase/supabase/docker && docker-compose -f docker-compose-fixed.yml up -d"

echo Waiting for services to start...
timeout /t 30 /nobreak

echo Checking service status...
echo y | plink -ssh -pw Nomar123 root@154.26.155.207 "docker ps"

echo Step 6: Updating application configuration...
echo y | plink -ssh -pw Nomar123 root@154.26.155.207 "cat > /var/www/healthscribe/.env.local << 'EOF'
...
echo y | plink -ssh -pw Nomar123 root@154.26.155.207 "curl -H 'apikey: eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxvY2FsaG9zdCIsInJvbGUiOiJhbm9uIiwiaWF0IjoxNjQwOTk1MjAwLCJleHAiOjE5NTYzNTUyMDB9.EGIM96RAZx35lJzdJsyH-qQwv8Hdp7fsn3W0YpN81IU' http://154.26.155.207:8000/auth/v1/token"

echo y | plink -ssh -pw Nomar123 root@154.26.155.207 "pm2 restart all --update-env"
echo y | plink -ssh -pw Nomar123 root@154.26.155.207 "pm2 status"
