...
async function main() {
  ...
  try {
    ...
    console.log('🔧 Creating Transcriptions and Finishing Setup...\n');
    
    // Create transcriptions
    console.log('1. Creating 29 transcriptions:\n');
    await executeCommand(conn, `
docker exec supabase-db-e088wwks88k8k48sccg8gk0o psql -U postgres -d postgres <<'SQL'
DELETE FROM public.transcriptions;

INSERT INTO public.transcriptions (user_id, file_name, transcription_text, status, doctor_name, patient_name, document_type, file_size, duration, created_at)
SELECT 
  (SELECT id FROM auth.users WHERE email = 'omars14@gmail.com'),
...
`);
        ...
        console.log('5. Admin Panel: /dashboard/admin/users');
        console.log('');
      } else {
        console.log('❌ API not returning transcriptions\n');
      }
    }
