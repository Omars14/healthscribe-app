# PowerShell script to fix Supabase
$ErrorActionPreference = "Continue"

Write-Host "🔧 COMPREHENSIVE SUPABASE FIX - STARTING" -ForegroundColor Green
...
function Execute-SSHCommand {
    ...
    try {
        $result = ssh root@154.26.155.207 $command 2>&1
...
Write-Host "Step 1: Cleaning up broken services..." -ForegroundColor Green
...
Write-Host "Step 2: Creating environment configuration..." -ForegroundColor Green
$envConfig = @"
...
Write-Host "Step 3: Creating Docker Compose configuration..." -ForegroundColor Green
$dockerCompose = @"
version: '3.8'
services:
  db:
...
Write-Host "Step 4: Creating Kong configuration..." -ForegroundColor Green
$kongConfig = @"
_format_version: "1.1"
services:
  - name: auth-v1-open
...
Write-Host "Step 5: Starting Supabase services..." -ForegroundColor Green
...
Write-Host "Step 6: Updating application configuration..." -ForegroundColor Green
$appConfig = @"
...
Write-Host "Step 7: Testing endpoints and restarting application..." -ForegroundColor Green
...
Write-Host "🎉 COMPREHENSIVE SUPABASE FIX COMPLETED!" -ForegroundColor Green
Write-Host "Your self-hosted Supabase should now be working properly." -ForegroundColor Green
Write-Host "Test your application at: http://www.healthscribe.pro" -ForegroundColor Green
