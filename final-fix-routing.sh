...
echo "🔧 FINAL FIX - RESOLVING ROUTING ISSUES"
echo "======================================="
...
# Step 1: Check current auth service status
echo "Step 1: Checking auth service status..."
docker ps | grep supabase-auth
if [ $? -ne 0 ]; then
    echo "❌ Auth service not running, restarting..."
    docker start supabase-auth
    sleep 10
fi

docker ps | grep supabase-auth
check_error "Auth service check"

# Step 2: Test auth service directly
echo "Step 2: Testing auth service directly..."
AUTH_TEST=$(curl -s -w "%{http_code}" -H "apikey: eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxvY2FsaG9zdCIsInJvbGUiOiJhbm9uIiwiaWF0IjoxNjQwOTk1MjAwLCJleHAiOjE5NTYzNTUyMDB9.EGIM96RAZx35lJzdJsyH-qQwv8Hdp7fsn3W0YpN81IU" \
  http://localhost:9999/health)

if [[ "$AUTH_TEST" == *"200"* ]]; then
    echo "✅ Auth service responding on port 9999"
else
    echo "⚠️ Auth service not responding on /health, trying /token..."
    AUTH_TEST=$(curl -s -w "%{http_code}" http://localhost:9999/token)
    if [[ "$AUTH_TEST" == *"200"* ]] || [[ "$AUTH_TEST" == *"400"* ]]; then
        echo "✅ Auth service responding on port 9999"
    else
        echo "❌ Auth service not responding properly"
        docker logs supabase-auth --tail 10
    fi
fi

# Step 3: Check Kong configuration
echo "Step 3: Checking Kong configuration..."
docker exec supabase-kong kong config -c /home/kong/kong.yml | head -20

# Step 4: Update Kong configuration to use localhost
echo "Step 4: Updating Kong configuration for proper routing..."
...
# Step 5: Reload Kong configuration
echo "Step 5: Reloading Kong configuration..."
docker exec supabase-kong kong reload

# Step 6: Update Nginx configuration to use localhost
...
echo "Step 8: Testing all endpoints after fixes..."
...
echo ""
echo "🎉 FINAL ROUTING FIX COMPLETED!"
echo "==============================="
echo "✅ Auth service running and accessible"
echo "✅ Kong configuration updated with proper routing"
